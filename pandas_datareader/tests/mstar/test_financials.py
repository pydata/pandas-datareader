...
from pandas_datareader.data import BalanceSheetReader, \
...
class TestMorningstarFinancials(object):
    ...
    def multi_symbol_keyratios(self):
        ...
        key_ratios = kr.read()
        gm = key_ratios["2008-06-01"]["DIS"]["Financials - Summary"][
            "Gross Margin %"]
    ...
    def invalid_args(self):
        ...
        pytest.raises(TypeError, BalanceSheetReader, "MSFT", "A",
                      rounding=rounding)
        pytest.raises(ValueError, BalanceSheetReader, "MSFT", "A",
                      periods=periods)
    ...
    def single_symbol_financials(self):
        ...
        assert (all([i for i in [BS_A, BS_Q, CF_A, CF_Q, IS_A, IS_Q] if
                     len(i[i.keys()[0]]) == 1]))
    ...
    def check_restated_vs_reported(self):
        BS_A = BalanceSheetReader(symbols="TWTR", freq="A",
                                  dataType="A").read()
        assert ("TTM" in [i for i in BS_A.keys()])
        BS_Ax = BalanceSheetReader(symbols="TWTR", freq="A",
                                   dataType="R").read()
