...
class BaseFinancialsReader(object):
    ...
    def __init__(self, symbols, freq, report, **kwargs):
        ...
        if len(report) != 2:
            raise AttributeError("Invalid report type. Must be in ['is','bs','cf']")
    ...
    def _check_optional_kwargs(self, **kwargs):
        ...
        if "rounding" in kwargs.keys() and type(kwargs.get("rounding")) is not int:
            raise TypeError("Invalid rounding argument. rounding digits must be int")

        if "periods" in kwargs.keys() and kwargs.get("periods") > 10:
            raise ValueError("Max number of periods is 10 when using a free account")

        if "dataType" in kwargs.keys() and kwargs.get("dataType") not in ["A", "R"]:
            raise ValueError("Invalid dtype. Must be either 'A'ctual or 'R'estated")

        if "view" in kwargs.keys() and kwargs.get("view") not in ["raw", "decimal", "percentage"]:
            raise AttributeError("invalid view method, must be ('raw', 'decimal' or 'percentage')")

        if "units" in kwargs.keys():
            if kwargs.get("units") not in [1, 2, 3, 4]:
                raise ValueError("Invalid units. Must be int value between 1-4. \nAccepted Values:\n"
        ...
        if "order" in kwargs.keys():
            if kwargs.get("order") not in ["asc", "desc"]:
                raise ValueError("Invalid ordering argument must be either, 'asc' or 'desc'")
            elif type(kwargs.get("order")) is not str:
                raise TypeError("Invalid ordering argument must be either, 'asc' or 'desc'")
    ...
    def _set_header(self, row):
        ...
        dates = [datetime.strptime(v, "%Y-%m").strftime("%Y-%m-%d") for v in row[1:len(row)]]
    ...
    def url_params(self):
        ...
        if self.freq in ["A", "Y"] and url_params.get("columnYear") > 5:
            raise ValueError("periods restricted to max 5 for annual data unless using paid account")
        elif self.freq == "Q" and url_params.get("columnYear") > 10:
            raise ValueError("periods restricted to max 10 for annual data unless using paid account")
        else:
    ...
    def _request(self, params):
        ...
        if resp.status_code == 200:
            if len(resp.text) > 0:
                ...
                print("Error in trying to collect data for %s" % self.request_params["t"])
    ...
    def read(self):
        ...
        if len(self.data_buffer) == 0:
            ...
            df = DataFrame([d[2:] for d in self.data_buffer],
                           index=MultiIndex.from_tuples([(d[0], d[1]) for d in self.data_buffer],
...
class KeyRatiosReader(object):
        ...
        for l in range(len(splitlines)):
            ...
            if l == 2:
                if len(self.data_buffer) > 0:
                    ...
                    lx = splitlines[l].split(",")
                    vals = [datetime.strptime(v, "%Y-%m").strftime("%Y-%m-%d") for v in lx[1:len(lx)-1]]
                        ...
                        if splitlines[l].find(",") > -1:
                            ...
                            if self._detect_date_row(row=lx):
                                ...
                                if row_type is "percent":
                                    ...
                                    formattedline = self._format_currency(row=lx, units=units)
                                elif row_type == "value":
    ...
    def _gen_index_tuples(self):
        return MultiIndex.from_tuples([(d[0], d[1], d[2]) for d in self.data_buffer],
    ...
    def read(self):
        ...
        if len(self.data_buffer) == 0:
            ...
            df = DataFrame(data=[d[3:] for d in self.data_buffer], index=self._gen_index_tuples(), columns=self.header)
